class Game
  ...
  def non_strike?(frame)
    frame[0] != 10
  end

  def non_spare?(frame)
    frame_completed?(frame) && frame[0] + frame[1] < 10
  end
  ...
  def roll(pins)
    ...
    raise Game::BowlingError if bonus_roll? && (non_strike?(last_frame) && non_spare?(last_frame))
    raise Game::BowlingError if bonus_roll? && (spare?(frame_before_last) && !frame_completed?(last_frame))
    raise Game::BowlingError if bonus_roll? && (strike?(frame_before_last) && last_frame[0] != 10 && pins == 10)

    if pins == 10
      @frames << [10, 0]
      @frames_allowance += 2 if tenth_frame?
      ...
      @frames_allowance += 1 if combo == 10 && tenth_frame?
    else
      add_new_frame(pins)
    end
  end
